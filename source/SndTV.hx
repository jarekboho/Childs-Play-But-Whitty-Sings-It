...
class SndTV
{
	...
	function create_(p:Snd, vartype:TVVar, to:Float, ?tp:h2d.Tweenie.TType, ?duration_ms:Float):TweenV
	{
		...
		if (pool.length == 0)
		{
			t = new TweenV(p, 0.0, 0.0, vartype, 1 / (duration_ms * fps / 1000),
				from, to, tp, 1, null, null);
		}
		else
		{
			t = pool.pop();
			t.reset(p, 0.0, 0.0, vartype, 1 / (duration_ms * fps / 1000),
				from, to, tp, 1, null, null);
		}
	...
	inline function onUpdate(t:TweenV, n:Float)
	{
		if (t.onUpdate != null)
			t.onUpdate(t);
	}

	inline function onEnd(t:TweenV)
	{
		if (t.onEnd != null)
			t.onEnd(t);
	}
